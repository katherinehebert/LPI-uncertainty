---
title: "HMSC vs. LPI"
runningheader: "DFO dataset example" # only for pdf output
subtitle: "DFO dataset example" # only for html output
date: "`r Sys.Date()`"
output:
...
# The DFO dataset (from 1995 to 2016)
...
# this is the LPI version with bootsrapped CI
ggplot() +
  geom_ribbon(data = lm_boot, aes(x = as.numeric(year), ymin = lwr.ci, ymax = upr.ci), alpha = .2) +
  geom_line(data = lm_boot, aes(x = as.numeric(year), y = mean))
```
