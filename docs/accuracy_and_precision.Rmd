...
```{r, fig.cap="Ability to capture the true LPI", fig.width=7, fig.height = 5}
# success_diff <- df_sc1 %>% group_by(direction, Process_error) %>%
#   mutate(meandiff = mean(true_diff_rlpiCI)) %>% 
#   ungroup() 
#   
# success_diff %>% 
#   ggplot() +
#   geom_tile(aes(x = direction, y = Process_error, fill = meandiff)) +
#   scale_fill_distiller(palette = "Reds", direction = 1, limits = c(0,.01)) +
#   labs(fill = "Truth's difference \nfrom confidence interval", y = "", x = "")
# 
# success %>% kable() %>% kable_styling(bootstrap_options = "striped")
ggplot(df_sc1) +
  geom_boxplot(aes(y = true_diff_rlpiCI, x = direction, fill = Process_error)) +
