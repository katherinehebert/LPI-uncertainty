...
df_cov %>% filter(Process_error == "0") %>%
  ...
  summarise(mean_accuracy = round(mean(accuracy_boot),3),
            se_accuracy = round(se(accuracy_boot), 3),
            mean_precision = round(mean(interval_diff),3),
            se_precision = round(se(interval_diff),3)) %>% 
  kableExtra::kable()
```
