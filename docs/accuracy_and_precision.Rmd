...
precision_success_plot <- function(df_subset, comparison_variable){
  
  ggplot(data = df_subset, 
         aes(x = get(comparison_variable), 
             fill = true_within_rlpiCI)) +
    geom_bar(stat = "count", position = "stack") +
    labs(y = "Frequency", 
         x = "", 
         title = "Ability to capture the truth",
         fill = "True LPI within CI?") +
    theme(legend.position = "bottom") +
    scale_fill_manual(values = c("#FB5B5A", "#003F5C"))
}
...
```{r}
precision_success_plot(filter(df_sc1, Process_error == "0"), "direction") 
```


```{r, include = FALSE, echo = FALSE}
(d + labs(title = "Trends")) / (a + b + c)
...
```{r}
precision_success_plot(filter(df_sc1, Process_error == "0.1"), "direction") 
```
...
```{r}
precision_success_plot(filter(df_sc1, Process_error == "0.2"), "direction") 
```
...
```{r clean-this-up, include= FALSE, eval=FALSE}
# import the lambdas (dt) from log-ratio of raw population sizes
sc1_dt <- lapply(temp, function(x) {readRDS(here::here(paste0("outputs/",x,"_l_dtchain.RDS")))})
names(sc1_dt) <- temp
sc1_dt <- bind_rows(sc1_dt, .id = "scenario")
sc1$dt_chain <- sc1_dt$dt_chain

# import lambdas from the rlpi calculation
lambdas <- lapply(temp, function(x) {
  read_csv(here::here(paste0("outputs/rlpi/",x,"_pops_lambda.csv"))) %>%
    pivot_longer(cols = 4:ncol(.), names_to = "time", values_to = "rlpi")
  }
  )
names(lambdas) <- temp
lambdas <- bind_rows(lambdas, .id = "scenario")
sc1_dt$dt_rlpi <- lambdas$rlpi

dtemp <- lapply(temp, function(x) {
  read_csv(here::here(paste0("outputs/rlpi/",x,"_pops_dtemp.csv"))) %>%
    pivot_longer(cols = everything(), names_to = "time", values_to = "mu_rlpi")
  }
  )
names(dtemp) <- temp
dtemp <- bind_rows(dtemp, .id = "scenario")

# remove rows for first time step, since this is a baseline value
#sc1 <- filter(sc1, time != 1)

# get geometric mean dt of each distribution
means <- group_by(sc1_dt, scenario, pop, time) %>%
  # per "species"
  summarise(mu_raw = EnvStats::geoMean(exp(dt_chain), na.rm = TRUE) # log-ratio of the raw population sizes
            ) %>% ungroup() %>%
  #"overall"
  group_by(scenario, time) %>%
  summarise(mu_raw = EnvStats::geoMean(mu_raw, na.rm = TRUE))

means_overall <- group_by(sc1_dt, scenario, pop) %>%
  summarise(mu_raw = EnvStats::geoMean(exp(dt_chain), na.rm = TRUE),
            mu_pred = EnvStats::geoMean(exp(dt_rlpi), na.rm = TRUE),
            ) %>% ungroup() %>%
  group_by(scenario) %>%
  summarise(
    mu_raw = log(EnvStats::geoMean(mu_raw)),
    mu_pred = log(EnvStats::geoMean(mu_pred))
  )

# assign these rlpi dt to the data frame
means$mu_rlpi <- 10^dtemp$mu_rlpi
means[which(means$time == 1), "mu_rlpi"] = 1
...
ggplot(sc1_dt)+
  geom_histogram(aes(x = dt_chain, fill = "raw", col = "raw"), 
    ...
    geom_histogram(aes(x = dt_rlpi, fill = "predicted", col = "predicted"), 
               alpha = .3, lwd = .1) +
  geom_vline(data = means_overall, aes(xintercept = mu_raw, 
                               col = "raw"), lwd = .5) +
  geom_vline(data = means_overall, aes(xintercept = mu_pred, 
                               col = "predicted"), lwd = .5) +
  scale_fill_manual(values = pal_compare) + 
  scale_color_manual(values = pal_compare) +
  facet_wrap(~scenario) +
...
a <- accuracy_plot(dplyr::filter(df_lag, Process_error == "0"), 
  ...
  theme(legend.position = "none") + scale_color_manual(values = pal_lags) + scale_fill_manual(values = pal_lags)
...
a <- accuracy_plot(dplyr::filter(df_lag, Process_error == "0.1"), 
                   comparison_variable = "direction", 
                   colour_variable = "Lag") + 
                    facet_wrap(~interaction) + #labs(x = "Lag") +
