...
ggsave("~/Documents/GitHub/LPI-sensitivity/figures/fig2_direction.png", width = 7.6, height = 2.78)
...
a + b
ggsave("~/Documents/GitHub/LPI-sensitivity/figures/fig3_covariance.png", width = 7.6, height = 3.78)
```
...
```{r}
pal_lags <- c("#08306b", "#2171b5", "#6baed6")
```
...
# plotly::ggplotly(a)
# plotly::ggplotly(b)
ggsave("~/Documents/GitHub/LPI-sensitivity/figures/fig4_lag.png", width = 9, height = 5)
```
...
df_lag %>%
  ...
  summarise(mean_se(interval_diff)) %>%
ggplot(aes(x = Process_error, y = y, col = Lag)) +
  ...
  labs(y = "Difference in interval width", 
         ...
         col = "Lag") +
 theme(legend.position = "bottom") 
ggsave("~/Documents/GitHub/LPI-sensitivity/figures/figsupp_lag.png", width = 5.71, height = 7.42)
```
